...
def get_filter_user(users):
    ...
    for i in range(len(users)):
        if (int(users[i]['math_score']) + int(users[i]['science_score']) + int(users[i]['english_score']) + int(users[i]['social_score'])) // 4 >= 70:
            arr.append({'name':users[i]['name'],'age':users[i]['age']})
            
    return arr
        
    
pprint.pprint(get_filter_user(users))
